...
public:
    ...
    void writeQ2DGridCSV(int err, bool dotitles = true, std::string ztitle = "z") {
        auto runLabel = [&]() {
            std::string label = to_string(int(T_in)) + "K_" + to_string(int(p_out/1e5)) + "Bar_";
            if(is_sccm)
            {
                label += to_string(int(m_SCCM)) + "sccm";
            }
            else {
                label += to_string(double(m_inletVel)) + "vel";
            }
            return label;
        };
        ...
        if (err == (-1))
        {
            csvFilename = "Err_q2d_grid_" + runLabel() + ".csv";
            manifestFilename = "Err_q2d_grid_manifest_" + runLabel() + ".json";
        } else if (err == 0) {
            csvFilename = "q2d_grid_" + runLabel() + ".csv";
            manifestFilename = "q2d_grid_manifest_" + runLabel() + ".json";
        } else {
            csvFilename = "steadysolve_grid_" + to_string(int(err)) + "_" + runLabel() + ".csv";
            manifestFilename = "steadysolve_grid_manifest_" + to_string(int(err)) + "_" + runLabel() + ".json";
        }

        int np = nPoints();
        int nc = nComponents();
        vector_double x(np * nc, 0.0);
        for (int n = 0; n < np; n++) {
            for (int m = 0; m < nc; m++) {
                x[index(m, n)] = m_sim->value(1, m, n);
            }
        }

        updateProperties(npos, x.data(), 0, np - 1);
        if(m_radialPoints > 1)
        {
            interfaceFlux(x.data(), 0, np - 1);
        }

        const double nan = std::numeric_limits<double>::quiet_NaN();
        auto partIndex = [&](size_t i) {
            return (i < m_lumenPoints) ? 0 : 1;
        };
        auto isReactorCell = [&](size_t i) {
            return i < m_radialPoints;
        };
        auto regionName = [&](size_t i) {
            if(i < m_lumenPoints)
            {
            ...
            return std::string("annular_channel");
        };
        auto radialCenter = [&](size_t i) {
            if(isReactorCell(i))
            {
                return m_r[i];
            }
            double channelInnerRadius = m_solveSupport ? m_Rout : m_Rin;
            return channelInnerRadius + 0.5 * (m_rChannel - channelInnerRadius);
        };
        auto radialInner = [&](size_t i) {
            if(isReactorCell(i))
            {
                return (i == 0) ? 0.0 : m_rWall[i - 1];
            }
            return m_solveSupport ? m_Rout : m_Rin;
        };
        auto radialOuter = [&](size_t i) {
            if(isReactorCell(i))
            {
                return m_rWall[i];
            }
        ...
        auto axialLeft = [&](size_t j) {
            return (j == 0) ? 0.0 : m_zWall[j - 1];
        };
        auto axialRight = [&](size_t j) {
            return m_zWall[j];
        };
        auto writeCsvValue = [&](std::ofstream& f, double value) {
            f << value;
        };
        ...
        if (dotitles) {
            f << "axial_index,radial_index,region," << ztitle << ",r,z_left,z_right,r_inner,r_outer,";
            f << "pressure,density,mean_molecular_weight,cp_mass,velocity_axial,";
            if(m_solveRadialVel)
            {
                f << "velocity_radial,";
            }
            f << "gas_temperature,solid_temperature,viscosity,thermal_conductivity,";
            f << "porosity,tortuosity,specific_surface_area,particle_diameter,pore_radius";
            for (size_t k = 0; k < m_nsp; k++) {
                f << ",Y_" << m_thermo->speciesName(k);
            }
            for (size_t k = 0; k < m_nsp; k++) {
                f << ",X_" << m_thermo->speciesName(k);
            }
            for (size_t k = 0; k < m_nspSurf; k++) {
                f << ",theta_" << m_surf->speciesName(k);
            }
            if(m_solveSupport)
            {
                for (size_t k = 0; k < m_nspSurf_2; k++) {
                    f << ",theta_support_" << m_surf_2->speciesName(k);
                }
            }
            for (size_t k = 0; k < m_nsp; k++) {
                f << ",wdot_" << m_thermo->speciesName(k);
            }
            for (size_t k = 0; k < m_nspSurf; k++) {
                f << ",sdot_" << m_surf->speciesName(k);
            }
            if(m_solveSupport)
            {
                for (size_t k = 0; k < m_nspSurf_2; k++) {
                    f << ",sdot_support_" << m_surf_2->speciesName(k);
                }
            }
            for (size_t k = 0; k < m_nsp; k++) {
                f << ",sdot_gas_" << m_thermo->speciesName(k);
            }
            for (size_t k = 0; k < m_nsp; k++) {
                f << ",axial_flux_" << m_thermo->speciesName(k);
            }
            for (size_t k = 0; k < m_nsp; k++) {
                f << ",radial_flux_" << m_thermo->speciesName(k);
            }
            f << std::endl;
        }
        ...
        for (int n = 0; n < np; n++) {
            for (size_t i = 0; i < m_totRadialPoints; i++) {
                bool reactorCell = isReactorCell(i);
                int part = reactorCell ? partIndex(i) : 0;
                size_t surfnsp = (part == 0) ? m_nspSurf : m_nspSurf_2;
                double rowViscosity = nan;
                double rowThermalConductivity = nan;
                if(reactorCell)
                {
                    getWdot(x.data(), n, i);
                    getSdot(x.data(), n, i);
                    ...
                    {
                        setGas(x.data(), n, i);
                        rowViscosity = m_trans->viscosity();
                        rowThermalConductivity = m_trans->thermalConductivity();
                    }
                    else {
                        setGasSupport(x.data(), n, i);
                        rowViscosity = m_trans_2->viscosity();
                        rowThermalConductivity = m_trans_2->thermalConductivity();
                    }
                }

                f << n << "," << i << "," << regionName(i) << ",";
                f << z(n) << "," << radialCenter(i) << ",";
                f << axialLeft(n) << "," << axialRight(n) << ",";
                f << radialInner(i) << "," << radialOuter(i) << ",";

                writeCsvValue(f, reactorCell || m_do_channel ? m_press(n,i) : nan); f << ",";
                writeCsvValue(f, reactorCell || m_do_channel ? m_rho(n,i) : nan); f << ",";
                writeCsvValue(f, reactorCell ? m_wtm(n,i) : nan); f << ",";
                writeCsvValue(f, reactorCell ? m_cp(n,i) : nan); f << ",";
                writeCsvValue(f, reactorCell || m_do_channel ? u(x.data(), n, i) : nan); f << ",";
                if(m_solveRadialVel)
                {
                    if(reactorCell)
                    {
                        int ind = (i < m_lumenPoints) ? i * nEq_lumen : nv_lumen + (i - m_lumenPoints) * nEq_support;
                        f << x[index(c_offset_V[part] + ind, n)];
                    }
                    else {
                        f << nan;
                    }
                ...
                writeCsvValue(f, reactorCell || m_do_channel ? T(x.data(), n, i) : nan); f << ",";
                writeCsvValue(f, reactorCell ? Ts(x.data(), n, i) : nan); f << ",";
                writeCsvValue(f, rowViscosity); f << ",";
                writeCsvValue(f, rowThermalConductivity); f << ",";
                writeCsvValue(f, reactorCell ? m_porosity[part] : nan); f << ",";
                writeCsvValue(f, reactorCell ? m_tortuosity[part] : nan); f << ",";
                writeCsvValue(f, reactorCell ? m_area2Vol[part] : nan); f << ",";
                writeCsvValue(f, reactorCell ? m_Dp[part] : nan); f << ",";
                writeCsvValue(f, reactorCell ? m_PoreRad[part] : nan);

                for (size_t k = 0; k < m_nsp; k++) {
                    f << ",";
                    writeCsvValue(f, reactorCell ? Y(x.data(), k, n, i) : nan);
                }
                for (size_t k = 0; k < m_nsp; k++) {
                    f << ",";
                    writeCsvValue(f, reactorCell ? m_xMole(k, cellIndex(n,i)) : nan);
                }
                for (size_t k = 0; k < m_nspSurf; k++) {
                    f << ",";
                    writeCsvValue(f, (reactorCell && part == 0) ? surfY(x.data(), k, n, i) : nan);
                }
                if(m_solveSupport)
                {
                    for (size_t k = 0; k < m_nspSurf_2; k++) {
                        f << ",";
                        writeCsvValue(f, (reactorCell && part == 1) ? surfY(x.data(), k, n, i) : nan);
                    }
                }
                for (size_t k = 0; k < m_nsp; k++) {
                    f << ",";
                    writeCsvValue(f, reactorCell ? m_wdot(k, cellIndex(n,i)) : nan);
                }
                for (size_t k = 0; k < m_nspSurf; k++) {
                    f << ",";
                    writeCsvValue(f, (reactorCell && part == 0) ? m_sdot(k, cellIndex(n,i)) : nan);
                }
                if(m_solveSupport)
                {
                    for (size_t k = 0; k < m_nspSurf_2; k++) {
                        f << ",";
                        writeCsvValue(f, (reactorCell && part == 1) ? m_sdot(k, cellIndex(n,i)) : nan);
                    }
                }
                for (size_t k = 0; k < m_nsp; k++) {
                    f << ",";
                    writeCsvValue(f, reactorCell ? m_sdot(k + surfnsp, cellIndex(n,i)) : nan);
                }
                for (size_t k = 0; k < m_nsp; k++) {
                    f << ",";
                    writeCsvValue(f, reactorCell ? m_flux(k, cellIndex(n,i)) : nan);
                }
                for (size_t k = 0; k < m_nsp; k++) {
                    f << ",";
                    writeCsvValue(f, (reactorCell && m_radialPoints > 1) ? m_interfaceFlux(k, cellIndex(n,i)) : nan);
                }
                f << std::endl;
            }
        }
        ...
        auto writeJsonStringArray = [&](std::ofstream& out, const std::string& key, const std::vector<std::string>& values, bool trailingComma) {
            out << "  \"" << key << "\": [";
        ...
        std::vector<std::string> surfaceSpecies;
        std::vector<std::string> supportSurfaceSpecies;
        for(size_t k = 0; k < m_nsp; k++)
        {
            gasSpecies.push_back(m_thermo->speciesName(k));
        }
        for(size_t k = 0; k < m_nspSurf; k++)
        {
            surfaceSpecies.push_back(m_surf->speciesName(k));
        }
        if(m_solveSupport)
        {
            for(size_t k = 0; k < m_nspSurf_2; k++)
            {
                supportSurfaceSpecies.push_back(m_surf_2->speciesName(k));
            }
        }

        std::ofstream manifest(manifestFilename);
        manifest << "{\n";
        manifest << "  \"csv_file\": \"" << jsonEscape(csvFilename) << "\",\n";
        manifest << "  \"input_file\": \"" << jsonEscape(m_inputFilename) << "\",\n";
        manifest << "  \"format\": \"long_csv_one_row_per_z_r_cell\",\n";
        manifest << "  \"dimensions\": {\n";
        manifest << "    \"axial_points\": " << m_axialPoints << ",\n";
        manifest << "    \"lumen_points\": " << m_lumenPoints << ",\n";
        manifest << "    \"support_points\": " << m_supportPoints << ",\n";
        manifest << "    \"reactor_radial_points\": " << m_radialPoints << ",\n";
        manifest << "    \"total_radial_points\": " << m_totRadialPoints << "\n";
        manifest << "  },\n";
        manifest << "  \"region_counts\": {\n";
        manifest << "    \"lumen\": " << m_lumenPoints << ",\n";
        manifest << "    \"support\": " << m_supportPoints << ",\n";
        manifest << "    \"annular_channel\": " << (m_do_channel ? 1 : 0) << "\n";
        manifest << "  },\n";
        manifest << "  \"solver_flags\": {\n";
        manifest << "    \"solve_energy\": " << m_do_energy << ",\n";
        manifest << "    \"solve_solid\": " << m_solveSolid << ",\n";
        manifest << "    \"solve_support\": " << m_solveSupport << ",\n";
        manifest << "    \"solve_radial_velocity\": " << m_solveRadialVel << ",\n";
        manifest << "    \"solve_membrane\": " << m_solveMem << ",\n";
        manifest << "    \"solve_annular_channel\": " << m_do_channel << ",\n";
        manifest << "    \"transport_model\": \"" << (m_tranModel == m_DGM ? "DGM" : "FICK") << "\"\n";
        manifest << "  },\n";
        manifest << "  \"mechanisms\": {\n";
        manifest << "    \"lumen_gas_file\": \"" << jsonEscape(m_filenameGas[0]) << "\",\n";
        manifest << "    \"lumen_gas_phase\": \"" << jsonEscape(m_gasPhase[0]) << "\",\n";
        manifest << "    \"lumen_surface_file\": \"" << jsonEscape(m_filenameSurf[0]) << "\",\n";
        manifest << "    \"lumen_surface_phase\": \"" << jsonEscape(m_surfPhase[0]) << "\",\n";
        manifest << "    \"support_gas_file\": \"" << jsonEscape(m_filenameGas[1]) << "\",\n";
        manifest << "    \"support_gas_phase\": \"" << jsonEscape(m_gasPhase[1]) << "\",\n";
        manifest << "    \"support_surface_file\": \"" << jsonEscape(m_filenameSurf[1]) << "\",\n";
        manifest << "    \"support_surface_phase\": \"" << jsonEscape(m_surfPhase[1]) << "\"\n";
        manifest << "  },\n";
        manifest << "  \"units\": {\n";
        manifest << "    \"z\": \"m\", \"r\": \"m\", \"pressure\": \"Pa\", \"density\": \"kg/m^3\",\n";
        manifest << "    \"mean_molecular_weight\": \"kg/kmol\", \"cp_mass\": \"J/kg/K\",\n";
        manifest << "    \"velocity_axial\": \"m/s\", \"velocity_radial\": \"m/s\",\n";
        manifest << "    \"gas_temperature\": \"K\", \"solid_temperature\": \"K\",\n";
        manifest << "    \"viscosity\": \"Pa*s\", \"thermal_conductivity\": \"W/m/K\",\n";
        manifest << "    \"specific_surface_area\": \"1/m\", \"particle_diameter\": \"m\",\n";
        manifest << "    \"pore_radius\": \"m\", \"wdot\": \"kmol/m^3/s\",\n";
        manifest << "    \"sdot\": \"kmol/m^2/s\", \"axial_flux\": \"kg/m^2/s\",\n";
        manifest << "    \"radial_flux\": \"kg/m^2/s\"\n";
        manifest << "  },\n";
        writeJsonStringArray(manifest, "gas_species", gasSpecies, true);
        writeJsonStringArray(manifest, "surface_species", surfaceSpecies, true);
        writeJsonStringArray(manifest, "support_surface_species", supportSurfaceSpecies, false);
        manifest << "}\n";
    }
...
protected:
    ...
    // Provenance for machine-readable exports
    std::string m_inputFilename;
    std::string m_filenameGas[2];
    std::string m_filenameSurf[2];
    std::string m_gasPhase[2];
    std::string m_surfPhase[2];
...
public:
    ...
    virtual doublereal eval(doublereal a) const
    {
        ...
        main_ptr->writeQ2DGridCSV(main_ptr->refine_count);
        return 0;
    }

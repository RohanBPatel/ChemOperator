	...
	m_nspSurf(ip_ptr->m_nspSurf[0])
{
    ...
    for(int i = 0; i < 2; i++)
    {
        m_filenameGas[i] = ip_ptr->m_filenameGas[i];
        m_filenameSurf[i] = ip_ptr->m_filenameSurf[i];
        m_gasPhase[i] = ip_ptr->m_gasPhase[i];
        m_surfPhase[i] = ip_ptr->m_surfPhase[i];
    }
